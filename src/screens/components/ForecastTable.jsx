...
//   const now = new Date();
//   return (
//     (yearNum - now.getFullYear()) * 12 + (monthIdx - now.getMonth())
//   );
// }
...
//         <DialogContentText
//           sx={{ mb: 2, color: "text.primary", fontSize: 14 }}
//         >
//           {message}
//         </DialogContentText>
//         {editedDetails && (
...
//   const now = new Date();
//   const nowLabel = now.toLocaleString("default", {
//     month: "short",
//     year: "2-digit",
//   });
...
//     message: "",
//   });
//   const [highlightEditableCells, setHighlightEditableCells] =
//     useState(false);
...
//   const realWidth =
//     STICKY_W + visibleColumns.length * COL_W;
...
//     return (
//       STICKY_W + Math.max(totalCols, 1) * COL_W
//     );
...
//     const currentKey =
//       today.getFullYear() * 12 + today.getMonth();
//     const sortedMonths = (columns.length
//       ? columns
//       : visibleColumns
//     ).filter((label) => {
//       const [mon, yr] = label.split(" ");
...
//       today.getFullYear() * 12 + today.getMonth();
//     const set = new Set();
//     (columns.length ? columns : visibleColumns).forEach(
//       (label) => {
//         const [mon, yr] = label.split(" ");
//         const monthIdx = MONTH_MAP[mon.toUpperCase()];
//         if (monthIdx !== undefined) {
...
//         open: true,
//         message:
//           "Please select at least one SKU to edit consensus.",
//       });
//       return false;
//     }
...
//           const allowedFuture4 = new Set(
//             futureLabelsSorted.slice(0, 4)
//           );
...
//   const [actualLatestMonth, setActualLatestMonth] =
//     useState(null);
...
//       result.push(row);
//       if (row === "ML Forecast") {
//         if (optionalRows.includes("Sales"))
//           result.push("Sales");
//         if (
//           optionalRows.includes(
//             "Promotion / Marketing"
//           )
//         )
//           result.push(
...
//     try {
//       await updateConsensusForecastAPI(
//         pendingPayload
//       );
...
//     } catch (err) {
//       console.error(
//         "Consensus update failed:",
//         err
//       );
//       alert(
//         "Failed to update consensus forecast"
//       );
//       setConfirmationDialog({
//         open: false,
//         month: null,
...
//         open: true,
//         message:
//           "Nothing to download — no table data available.",
//       });
//       return;
//     }
//     const blob = new Blob(
...
//       const index =
//         period === "M"
//           ? findNearestIndexMonthly(
//               visibleColumns
//             )
//           : findNearestIndexWeekly(
//               visibleColumns
//             );
...
//                 background: "#DEE2E6",
//                 zIndex:
//                   Z_INDEX_LAYERS.STICKY_HEADER_COLUMN,
//                 fontWeight: 700,
//                 fontSize: 14,
//                 textAlign: "left",
//                 padding:
...
export default function ForecastTable({
  ...
  const chartColumns = useMemo(() => visibleColumns, [visibleColumns]);
    ...
    async (force = false) => {
      ...
      try {
        ...
        const mapeValues = raw
          .map((item) => Number(item.avg_mape))
          .filter((val) => !isNaN(val));
        const avgMapeVal = mapeValues.length
          ? mapeValues.reduce((a, b) => a + b, 0) / mapeValues.length
          : null;
        setAvgMape(avgMapeVal);
        ...
        if (period === "M") {
          ...
          // limit editable consensus to next 4 future weeks
          const nowStart = startOfISOWeek(new Date());
          const futureLabelsSorted = labels
  ...
  const handleConfirmationSubmit = async () => {
    ...
    try {
      ...
      await fetchForecastData(true); // force refresh with new data
    } catch (err) {
  ...
  const scrollToCurrentPeriod = useCallback(() => {
    ...
    const targetLeft =
      STICKY_W +
      index * COL_W -
      Math.max(0, (viewport.clientWidth - COL_W) / 2);

    const left = Math.max(
      0,
      Math.min(targetLeft, viewport.scrollWidth)
    );
    viewport.scrollTo({
      left,
      behavior: "smooth",
          ...
          {allRows.map((label, idx) => (
            ...
            >
              ...
              {visibleColumns.map((m) => {
                ...
                const locked =
                  period === "M" && isConsensusRow && isMonthLocked(m);
                const isAllowedMonth =
                  period === "M" &&
                  firstFutureMonth &&
  ...
  return (
    <>
            ...
            {["M", "W"].map((label) => (
              <Button
                ...
                sx={{
                  ...
                  lineHeight: "16px",
                  color:
                    period === label ? "#fff" : "#2563EB",
                  backgroundColor:
                    period === label ? "#2563EB" : "#fff",
                  border: "1px solid #2563EB",
                  textTransform: "none",
                  "&:hover": {
                    backgroundColor:
      ...
      <LockCommentPopover
        ...
        onClose={() =>
          setLockComment((prev) => ({ ...prev, open: false }))
        }
